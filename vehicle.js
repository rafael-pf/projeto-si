...
class Vehicle {
  constructor(obstacles, grid, gridSize, searchAlgorithm = null) {
    ...
    this.searchAlgorithm =
      searchAlgorithm || new BFS(grid, obstacles, gridSize);
  }
  ...
  findPath(target) {
    // Reset all data before starting new search
    this.searchAlgorithm.visited = [];
    this.searchAlgorithm.frontier = [];
    this.searchAlgorithm.path = null;
    this.searchAlgorithm.pathVis = null;
    this.path = null;
    this.pathVis = null;
  ...
  update() {
    ...
    if (this.path && this.path.length > 1) {
      ...
      let index = this.grid.findIndex(
        (g) => g.pos.x === target.x && g.pos.y === target.y,
      );

      if (this.grid[index].tag === 2) {
        //mud
        multiplier = 0.5;
      }
      if (this.grid[index].tag === 3) {
        //water
        multiplier = 0.25;
      }
